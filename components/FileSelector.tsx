...
export const FileSelector: React.FC<FileSelectorProps> = ({ isOpen, onClose, onSelect, title = "选择文件夹" }) => {
  ...
  const loadFiles = async (cid: string) => {
    ...
    try {
      // 调用真实 API
      const data = await api.get115Files(cid);
      
      if (data && data.files) {
        // 过滤掉非文件夹的文件（假设我们只选目录）
        // 注意：根据实际 API 返回结构调整，这里假设后端已处理好或返回 is_dir
        const dirs = data.files.filter((f: any) => f.is_dir || f.file_type === 0).map((f: any) => ({
            id: f.id || f.cid || f.file_id,
            name: f.name || f.n,
            is_dir: true,
            time: f.time || f.t || '' // 可选时间字段
        }));
        setFiles(dirs);
      } else {
        setFiles([]);
